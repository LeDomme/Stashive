...
async function load(collectionId:number,filter:api.InventoryFilter={}){loading.value=true;error.value=null;try{items.value=await api.listInventory(collectionId,filter)}catch(cause){error.value=cause instanceof ApiError?cause:new ApiError(0)}finally{loading.value=false}}
