...
async function loadCollection(): Promise<void> {
  ...
  if (collections.collection) {
    ...
    if (canManageMembers.value)
      await collections.loadMembers(collections.collection.id);
  }
}
...
function memberActionError(
  ...
  if (!(error instanceof ApiError))
    return `The member could not be ${action === "add" ? "added" : action === "remove" ? "removed" : "updated"}. Please try again.`;
  if (error.status === 409) {
  ...
  if (error.status === 404)
    return "The collection or requested user is unavailable.";
  if (error.status === 403)
...
async function updateMemberRole(
  ...
  const role = (event.target as HTMLSelectElement)
    .value as CollectionMember["role"];
  try {
    await collections.updateCollectionMember(
...
async function removeMember(): Promise<void> {
  ...
  memberError.value = "";
  try {
    await collections.deleteCollectionMember(
      collections.collection.id,
...
<template>
  ...
  >
    ...
    >
      ...
      <p v-if="collections.detailError.status === 403">
        You do not have permission to open this collection.
      </p>
      <p v-else-if="collections.detailError.status === 404">
    ...
    <template v-else-if="collections.collection">
      ...
      <section v-if="canEdit" class="panel settings-section">
        <div class="section-heading">
          ...
          <button type="button" class="button-secondary" @click="beginEditing">
            Edit details
          </button>
        </div>
      ...
      <section
        v-show="activeSection === 'sharing' && canManageMembers"
        class="members-panel panel"
      ...
      >
        ...
        <p v-if="collections.membersError" class="form-error" role="alert">
          Members could not be loaded. Please try again.
        </p>
        <p v-else-if="collections.members.length === 0" class="member-empty">
          No members have been added yet.
        </p>
        <ul v-else class="member-list" aria-label="Collection members">
          <li
            v-for="member in collections.members"
            :key="member.id"
            class="member-row"
          >
            <div>
            ...
            <button
              ...
              @click="removingMemberId = member.id"
            >
              Remove
            </button>
          </li>
        ...
        <p v-if="memberError" class="form-error" role="alert">
          {{ memberError }}
        </p>
        <section
          v-if="removingMemberId !== null"
          class="confirmation member-confirmation"
          aria-labelledby="remove-member-heading"
        >
          <h3 id="remove-member-heading">Remove this member?</h3>
          <p>They will no longer have access to this collection.</p>
          <div class="action-row">
            <button
              ...
              @click="removeMember"
            >
              {{ memberBusy ? "Removing…" : "Confirm remove" }}</button
            ><button
              type="button"
      ...
      <section
        v-show="activeSection === 'ownership' && canTransferOwnership"
        class="transfer-panel panel"
      ...
      >
        ...
        <form
          ...
          class="member-form"
          @submit.prevent="startTransferConfirmation"
        >
          <label
            >New owner username<input
        ...
        >
          <h3 id="confirm-transfer-heading">
            Transfer ownership to {{ transferUsername }}?
          </h3>
          <p>
            This user becomes the new owner. You will remain an admin member;
            the new owner will not also be a member.
          </p>
          <div class="action-row">
            <button
              type="button"
            ...
            ><button
              ...
              class="button-secondary"
              :disabled="transferring"
              @click="confirmingTransfer = false"
            >
              Cancel
            </button>
          </div>
      ...
      <form
        v-show="activeSection === 'general' && editing"
      ...
      >
        ...
        <label
          >Collection type<select v-model="type">
      ...
      <section
        v-show="activeSection === 'danger' && canDelete"
        class="confirmation panel danger-zone"
        aria-labelledby="delete-heading"
      >
        <p class="eyebrow">Danger zone</p>
        <h2 id="delete-heading">Delete collection</h2>
        <p>
          This permanently removes the collection and its related data. This
          cannot be undone.
        </p>
        <button
