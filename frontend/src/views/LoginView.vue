...
async function submit(): Promise<void> { try { await auth.signIn(username.value, password.value); await router.replace({ name: 'collections' }) } catch { error.value = 'Login failed. Check your credentials and try again.' } }
</script>
