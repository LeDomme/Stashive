...
async function submit(): Promise<void> { try { await auth.completeSetup(token.value, username.value, displayName.value, password.value); await router.replace({ name: 'collections' }) } catch { error.value = 'Setup failed. Check the token and entered credentials.' } }
</script>
