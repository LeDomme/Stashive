...
const props = withDefaults(defineProps<{
  ...
  emptyLabel?: string
  customLabel?: string
}>(), { allowEmpty: false, emptyLabel: 'Not specified', customLabel: 'Other / Custom' })
...
function select(value: string) { mode.value = value; if (value === customMode) emit('update:modelValue', custom.value || null); else emit('update:modelValue', value || null) }
function updateCustom(value: string) { custom.value = value; emit('update:modelValue', value || null) }
</script>
<template>
